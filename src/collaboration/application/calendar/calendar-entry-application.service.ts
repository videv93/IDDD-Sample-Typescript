...
export class CalendarEntryApplicationService {
  ...
  constructor(calendarEntryRepository: CalendarEntryRepository, collaboratorService: CollaboratorService) {
    this._calendarEntryRepository = calendarEntryRepository;
    this._collaboratorService = collaboratorService;
  }

  changeCalendarEntryDescription(tenantId: string, calendarEntryId: string, description: string) {
    const tenant = new Tenant(tenantId);
    let calendarEntry = this.calendarEntryRepository.calendarEntryOfId(tenant, new CalendarEntryId(calendarEntryId));
    calendarEntry.changeDescription(description);
    this.calendarEntryRepository.save(calendarEntry);
  }

  inviteCalendarEntryParticipant(tenantId: string, calendarEntryId: string, participantsToInvite: Set<string>) {
    const tenant = new Tenant(tenantId);
  ...
  relocateCalendarEntry(tenantId: string, calendarEntryId: string, location: string) {
    const tenant = new Tenant(tenantId);
    const calendarEntry = this.calendarEntryRepository.calendarEntryOfId(
      tenant,
      new CalendarEntryId(calendarEntryId),
